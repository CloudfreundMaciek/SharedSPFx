...
export default class BootstrapCarousel extends React.Component<IBootstrapCarouselProps, IBootstrapCarouselState> {
  ...
  constructor(props: IBootstrapCarouselProps) {
    ...
    this.createSlides(this.props.sp)
  ...
  public render(): React.ReactElement<IBootstrapCarouselProps> {
    ...
    return (
      ...
      >
        {(!this.state.errorMessage && slides && slides.length !== 0) ?
          // if there are slides && no error
          this.state.slides
            .map((v, i) =>
              <Carousel.Item key={i}>
          ...
          :
          // if slides are being loaded || there is none || error occurs
          <Carousel.Item>
  ...
  private async createSlides(sp: SPFI) {
